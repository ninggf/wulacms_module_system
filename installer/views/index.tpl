    ...
    <div class="container aside-xxl" style="min-width: 1000px">
        ...
        <section class="panel panel-default m-t-md bg-white wizard" id="install-wizard">
            ...
            <div class="step-content clearfix">
                <form action="{'system/installer/setup'|app}" class="m-b-sm m-t-sm" method="post" id="install-form">
...
<script type="text/javascript">
	layui.use(['wulaui', 'bootstrap'], function () {
		let tpl   = '<li class="list-group-item dy"><div class="clear"></div><div class="pull-right m-t-n"><i class="fa fa-refresh fa-spin"></i></div></li>',
			step1 = $('#step1');

		$(document).on('change', '.wizard', function (e, data) {
			if (!data || data.direction !== 'next') return;
			let item      = $(this).wizard('selectedItem');
			let $step     = $(this).find('.step-pane:eq(' + (item.step - 1) + ')');
			let validated = true;
			...
			if (!validated) {
				step1.animate({
					scrollTop: step1.prop("scrollHeight")
				}, 500);
				return e.preventDefault();
			}
		});

		var insWizard  = $('#install-wizard').on('changed', function () {
			console.log('adfasdf');
